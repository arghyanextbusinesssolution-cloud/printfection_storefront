  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-16">
      ...
      <div className="mb-10 border-b border-[#222] pb-6">
        ...
        <h1 className="text-3xl font-display font-black uppercase tracking-tighter text-white">Your Cart</h1>
      </div>
      ...
      <div className="space-y-4 mb-8">
        {items.map((item, index) => {
          ...
          return (
            <div key={index} className="bg-[#111] border border-[#222] p-6 transition-all duration-300 hover:border-[#333] flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
              <div className="flex gap-4 items-center">
                {/* Custom Design Preview Thumbnail */}
                {previewUrl ? (
                  <div className="w-16 h-20 bg-[#0d0d0d] border border-[#222] p-1 flex items-center justify-center overflow-hidden shrink-0">
                    <img src={previewUrl} alt={item.productName} className="max-w-full max-h-full object-contain" />
                  </div>
                ) : (
                  <div className="w-16 h-20 bg-[#0d0d0d] border border-[#222] flex items-center justify-center shrink-0">
                    <span className="material-symbols-outlined text-[#333] text-2xl">apparel</span>
                  </div>
                )}
                <div>
                  <h3 className="font-display font-bold text-white text-base uppercase tracking-tight">{item.productName}</h3>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {item.variants.filter((v) => v.quantity > 0).map((v) => (
                      <span key={v.size} className="bg-[#1a1a1a] border border-[#2a2a2a] px-2 py-0.5 font-mono text-[9px] text-[#aaa] uppercase">
                        {v.size}: {v.quantity}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex md:flex-col items-end justify-between w-full md:w-auto border-t md:border-t-0 border-[#222] pt-4 md:pt-0 gap-4">
                {item.pricingSnapshot ? (
                  <div className="text-left md:text-right">
                    <p className="font-mono text-[10px] text-[#555] uppercase tracking-[0.1em]">Total price</p>
                  ...
                  <span className="font-mono text-[9px] uppercase tracking-[0.1em] text-[#555]">Pricing pending</span>
                )}
                <button
                  onClick={() => removeMutation.mutate(index)}
                  className="font-mono text-[9px] uppercase tracking-[0.15em] text-red-500 hover:text-red-400 transition-colors flex items-center gap-1 border border-red-950 bg-red-950/20 px-2.5 py-1.5"
                >
                  Remove [X]
                </button>
              </div>
      ...
      {/* Cart Summary Card */}
      {totals && (
        <div className="bg-[#111] border border-[#222] p-6 mb-8 flex justify-between items-center">
          <div>
            <h3 className="font-display font-bold text-white text-sm uppercase tracking-tight">
              Total ({totals.itemCount} item{totals.itemCount !== 1 ? 's' : ''})
            </h3>
            <p className="font-mono text-[9px] uppercase tracking-[0.1em] text-[#555] mt-1">Including VAT &amp; standard shipping</p>
          </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          to="/bulk-order"
          className="border border-[#333] hover:border-white text-white font-mono text-[11px] uppercase tracking-[0.15em] px-6 py-3.5 transition-all text-center flex-1 sm:flex-none"
        >
          Add More Items
        </Link>
        <Link
          to="/checkout"
          className="bg-white hover:bg-neutral-200 text-black font-mono text-[11px] uppercase tracking-[0.15em] px-8 py-3.5 transition-all text-center flex-1 sm:flex-none"
        >
          Proceed to Checkout
        </Link>
        <Link
